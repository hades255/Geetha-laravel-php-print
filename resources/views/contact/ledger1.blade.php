...
@php
	...
	$bf = ($previous_purchases - $previous_payments + $previous_OB);
	$bal_due = ($previous_purchases - $previous_payments + $purchases - $payments + $previous_OB + $current_OB);
...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-50 f-left @endif">
	...
	<table class="table table-condensed text-left align-left no-border @if(!empty($for_pdf)) table-pdf @endif">
		...
		<tr>
			...
			<td>{{number_format($bf,  $currency_precision, session('currency')['decimal_separator'], session('currency')['thousand_separator'])}}
			</td>
		</tr>
		@if( $contact->type == 'supplier' || $contact->type == 'both')
			<tr>
				<td>@lang('report.total_purchase')</td>
				<td>{{number_format($purchases,  $currency_precision, session('currency')['decimal_separator'], session('currency')['thousand_separator'])}}
				</td>
			</tr>
		@endif
		@if( $contact->type == 'customer' || $contact->type == 'both')
			<tr>
				...
				<td id="total_invoice">{{number_format($purchases,  $currency_precision, session('currency')['decimal_separator'], session('currency')['thousand_separator'])}}
				</td>
			</tr>
		@endif
		...
		<tr>
			...
			<td id="total_paid">{{number_format($payments,  $currency_precision, session('currency')['decimal_separator'], session('currency')['thousand_separator'])}}
			</td>
		</tr>
		@if($total_discount > 0)
			<tr>
				<td>@lang('lang_v1.ledger_discount')</td>
				<td id="total_discount">{{number_format($total_discount,  $currency_precision, session('currency')['decimal_separator'], session('currency')['thousand_separator'])}}
			</tr>
		@endif
		<tr>
			<td><strong>@lang('lang_v1.balance_due')</strong></td>
			<td id="total_due">{{number_format($bal_due, $currency_precision, session('currency')['decimal_separator'], session('currency')['thousand_separator'])}}</td>
		</tr>
	</table>
</div>
<div class="col-md-12 col-sm-12 @if(!empty($for_pdf)) width-100 @endif">
	...
	<table class="table table-striped @if(!empty($for_pdf)) table-pdf td-border @endif" id="ledger_table">
		...
		@foreach($ledger_transactions as $data)
			...
			<tr>
				<td class="row-border">{{@format_date($data->paid_on)}}</td>
				<td>
				...
				<td>
					...
					@elseif($data->transaction_type == 'opening_balance') 
						Payment
					@elseif($data->transaction_type == 'cheque_opening_balance') 
						Opening Balance (Cheque)
					@elseif($data->transaction_type == 'sell')
					    @if($data->cheque_return_amount > 0)
						    Payment
						@else
						    @lang('lang_v1.sell') 
						@endif
					...
					@elseif($data->transaction_type == 'ledger_discount')
						@lang('lang_v1.ledger_discount')
					@endif
				</td>
				<td>{{$data->order_no}}</td>
				...
				<td>
					@if($data->transaction_type != 'ledger_discount')
				...
				@endif
				<td>
					@if(!empty($is_admin) && !empty($data->transaction_id) && $data->transaction_type == 'ledger_discount' && !$for_pdf)
					<br>
						<button type="button" class="btn btn-xs btn-primary btn-modal" data-href="{{route('ledger-discount.edit',$data->transaction_id)}}" data-container="#edit_ledger_discount_modal"><i class="fa fa-edit"></i></button>
						<button type="button" class="btn btn-xs btn-danger delete_ledger_discount" data-href="{{route('ledger-discount.destroy',$data->transaction_id)}}"><i class="fa fa-trash"></i></button>
					@endif
				</td>
			</tr>
			@if($data->interest>0)
