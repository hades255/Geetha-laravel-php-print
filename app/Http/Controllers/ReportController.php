...
$customers->prepend('All', 'all');
...

    {
        ...
        if ($request->ajax()) {
            ...
            // $data['total_sale_cost'] = $this->transactionUtil->getSaleCost($business_id, $start_date, $end_date)->total_sale_cost; @eng 12/2
            // $data['total_sales_on_cost'] = $this->transactionUtil->getSaleCost($business_id, $start_date, $end_date)->total_sale_cost; // @eng 13/2

            //Stocks

            // $data['opening_stock'] = !empty($opening_stock) ? $opening_stock : 0; // @eng 13/2

            // $data['closing_stock'] = $data['opening_stock'] + $data['total_purchase'] - $data['total_sale_cost']; //@eng 12/2
            // $data['closing_stock'] = $data['opening_stock'] + $data['total_purchase'] - $data['total_sales_on_cost']; //@eng 13/2
            // $data['closing_stock'] = !empty($closing_stock) ? $closing_stock : 0; // @eng 13/2
            ...
            // // @eng START 9/2 2359
            // $data['net_profit'] = $gross_profit - $data['total_expense'];
            // @eng END 9/2 2359
            
            
            // @eng START 15/2 
            // $data['total_sell'] = $this->getTotalSales();
            // $total_sales = 0;
            // $salesIncomeAccountGroup = AccountGroup::where('name', 'Sales Income Group')->where('business_id', $business_id)->first();
            // $accIds = Account::where('asset_type', $salesIncomeAccountGroup->id)->where('business_id', $business_id)->pluck('id');
            // foreach($accIds as $accId) {
            //     $total_sales += DB::table('account_transactions')
            ...
            // $cogsAccountGroup = AccountGroup::where('name', 'COGS Account Group')->where('business_id', $business_id)->first();
            // $accIds = Account::where('asset_type', $cogsAccountGroup->id)->where('business_id', $business_id)->pluck('id'); // @eng 15/2
            // foreach($accIds as $accId) {
            //     $total_sales_on_cost += DB::table('account_transactions')
            //         ->where('account_id', $accId)
            //         ->where('type', 'like', '%debit%')
            //         ->whereDate('operation_date', '>=', $start_date)
            //         ->whereDate('operation_date', '<=', $end_date)
            //         ->sum('amount');
            //     $previous_day_closing_stock -= DB::table('account_transactions')
            //         ->where('account_id', $accId)
            //         ->where('type', 'like', '%debit%')
            //         ->whereDate('operation_date', '<', $start_date)
            //         ->sum('amount');
            // }
            // $closing_stock -= $total_sales_on_cost;
            ...
            // $data['total_purchase'] = abs(Account::getStockGroupAccountBalanceByTransactionType('purchase', $start_date, $end_date));
            // @eng END 15/2
            $data['opening_stock'] = number_format($this->getPreviousDayStock($request, $start_date, $end_date), $currency_precision, '.', '');
...
//     public function getStockReport_(Request $request)
...
// WHERE transactions.status='final' AND transactions.type='purchase' AND (transactions.sub_type IS NULL OR transactions.sub_type='settlement') $location_filter

// AND TSL.variation_id=variations.id) as total_purchased"),
...
//                         $total_purchased = (float) $row->total_purchase;
//                     }
...
//                 'total_purchase_return', 'total_sold_return', 'total_purchased', 'unit_purchase_price', 'units'
...
//     }
    ...
    //             'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price', 'potential_profit','total_purchased'
    //         ];
...
(TSPL.quantity - TSPL.qty_returned) * transaction_sell_lines.unit_price_inc_tax )) AS total_sales'));
        ...
        if ($by == 'day') {
            ...
            foreach ($results as $result) {
                ...
                $profits[strtolower($result->day)]['total_sales'] = $result->total_sales;
            }
        ...
        if (request()->ajax()) {
            $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->where(function ($q) {
                    ...
                    $q->whereIn('transactions.type', ['opening_balance','advance_payment'])->orWhere('transactions.is_credit_sale', 1);
                })
...
<ul class="dropdown-menu dropdown-menu-right" role="menu">';
                        ...
                        if (auth()->user()->can("sell.view") || auth()->user()->can("direct_sell.access") || auth()->user()->can("view_own_sell_only")) {
                            ...
                            $html .= '<li><a href="#" data-href="' .url('/sales/outstanding_report').'?ref_no='.urlencode($row->payment_ref_no).'&paid_on='.urlencode($row->paid_on). '" class="btn-modal" data-container=".view_modal"><i class="fa fa-external-link" aria-hidden="true"></i> ' . __("messages.view") . '</a></li>';
                        }

                        if (auth()->user()->can("edit_received_outstanding")) {
            ...
            $rawColumns = ['final_total','paid_for', 'action', 'total_paid', 'total_remaining', 'payment_status', 'invoice_no', 'discount_amount', 'tax_amount', 'total_before_tax', 'shipping_status', 'payment_amount'];
                ...
                foreach ($data->data as $index => $row) {
                    ...
                    if ($currentContactId !== $row->contact_id) {
                        ...
                        if ($currentContactId !== null) {
                            ...
                            $modifiedData[] = [ 'name' => '<strong>Grand Total - '.$contact->name.':</strong>',
                            'route' => null,
                            'days_over' => null,
...
WHERE dip_readings.tank_id=fuel_tanks.id AND dip_readings.date_and_time < '$start_date'
        ...
        if(!empty(request()->print_only) && request()->print_only == 'true') {
            return view($page_view)->with(compact(
                'total_discount','location_details','dip_details','pump_operator_sales','cashiers', 'paginate','work_shift','day_diff',       'petro_module',          'business_locations',
                'work_shifts',
                'work_shift_id',
                'location_id',
                'sales',
                ...
                'cashiers_total_sales','pumps_total_sales',
                //Mahmoud Sabry
