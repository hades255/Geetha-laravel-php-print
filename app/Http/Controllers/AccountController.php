...
class AccountController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('account.create')
            ->with(compact('account_types', 'account_groups', 'asset_type_ids', 'accounts','parentAccounts','parentAccountsData'));
    }
    /**
    ...
    public function store(Request $request)
    {
        ...
        if (request()->ajax()) {
            try {
                $input = $request->only(['name', 'account_number', 'note', 'account_type_id', 'asset_type', 'is_main_account','is_need_cheque','show_in_balance_sheet']);
                $business_id = $request->session()->get('user.business_id');
                $user_id = $request->session()->get('user.id');
                $input['is_main_account'] = !empty($input['is_main_account']) ? $input['is_main_account'] : 0;
                $input['show_in_balance_sheet'] = !empty($input['is_main_account']) && $input['is_main_account']  ? $input['show_in_balance_sheet']: 0;
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->addColumn('description', function ($row) use ($business_id, $id, $currency_precision) {
                    ...
                    ##### ENDTASK ##########    
                    }elseif (empty($row->transaction->pump_operator_id) || $row->sub_type == 'deposit' ||  ($row->transaction->sub_type == 'credit_sale') || $row->transaction->sub_type == 'expense' || $row->transaction->sub_type == 'settlement') {
    ...
    public function edit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('account.edit')
                ->with(compact('account', 'account_types', 'balance', 'account_groups', 'account_access','selected_account_group', 'asset_type_ids', 'start_date', 'parent_accounts','parentAccountsData'));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            try {
                $user_id = $request->session()->get('user.id');
                $input = $request->only(['name', 'account_number','is_need_cheque', 'note','show_in_balance_sheet', 'account_type_id', 'parent_account_id', 'is_main_account', 'asset_type', 'increase_reduce', 'transaction_date']);
                $business_id = request()->session()->get('user.business_id');
                ...
                $account->show_in_balance_sheet = !empty($input['is_main_account']) && $input['is_main_account']  ? $input['show_in_balance_sheet']: 0;
    ...
    public function postFundTransfer(Request $request)
    {
        ...
        try {
            ...
            $accountName = "from : ".$fromAcc." to : ".$toAcc; 
    ...
    public function getBankAccountByGroupDP()
    {
        ...
        if($group_name == "accounts_receivable"){
            $accounts = Account::where('name','Accounts Receivable')->where('business_id', $business_id)->pluck('name','id');
        }else if($group_name == "accounts_payable"){
            $accounts = Account::where('name','Accounts Payable')->where('business_id', $business_id)->pluck('name','id');
        }else{
            if (!empty($group_id)) {
